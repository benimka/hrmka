                ...
                <div class="card-body">
                  ...
                  <form class="forms-sample" method="POST" action="<?php echo base_url(); ?>admin/users/save">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="exampleInputUsername1" name="name" value="<?php echo $value['name']; ?>">
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control" style="color:#000;" name="role_id" id="role_id">
                            <option value="">Select</option>
                            <?php
                                  $query = $this->db->query("SELECT * FROM sys_roles ");
                                  foreach ($query->result() as $row){
                                    $selected   = ($row->role_id == $value["role_id"]) ? ' selected="selected" ' : '';
                              ?>
                                <option name="role_id" id="role_id" value="<?php echo $row->role_id; ?>" <?=$selected?>><?php echo $row->role_name;?></option>
                              <?php
                          ...
                          <div class="col-sm-4">
                            <div class="form-check">
                              <label class="form-check-label">
                                <input type="radio" class="form-check-input" name="user_status" id="membershipRadios1" value="1"  <?php if ($value['user_status'] == '1') { echo "checked"; } ?>>
                                Active
                              </label>
                            </div>
                          </div>
